...
class ejecutarSQL {
    ...
    public static function consultar($query) {
        $conexion = ejecutarSQL::conectar(); // Guarda la conexión en una variable
        if (!$consul = mysqli_query($conexion, $query)) {
            die(mysqli_error($conexion) . ' Error en la consulta SQL ejecutada'); // Pasa la conexión a mysqli_error()
        }
        return $consul;
    }
}
